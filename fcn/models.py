...
def conv_helper(inputs, filters, kernel_size, name, do_dropout=True):
...
def upsample_helper(inputs, filters, scale, name):
    x = tf.layers.conv2d_transpose(inputs, nb_classes, (scale,scale), strides=(scale,scale), padding='valid', activation=None, name=name)
    return x
...
def fcn_32(sess, inputs, nb_classes):    
    ...
    conv7 = conv2d_helper(conv7, 4096, (1,1), name='conv7')
...
def fcn_8(sess, inputs, nb_classes):
    ...
    pool4 = get_vgg16_pooling_layer(4)
    pool4_score = conv2d_helper(pool4, nb_classes, (1,1), name='pool4_score')
    pool4_score_x2 = upsample_helper(pool4_conv7_score, nb_classes, 2, name='pool4_score_x2')

    # Get pool3 and class scores
    pool3 = get_vgg16_pooling_layer(3)
    pool3_score = conv2d_helper(pool4, nb_classes, (1,1), name='pool3_score')
